...
func (cm *ConnManager) Add(conn tiface.IConnection) error {
	...
	utils.GlobalLog.Infof("add %s connID=%d to connManager, current conn num=%d", conn.GetServer().GetServerType(), conn.GetConnID(), cm.Len())
	return nil
}
...
func (cm *ConnManager) Remove(conn tiface.IConnection) {
	...
	utils.GlobalLog.Warnf("remove %s connID=%d from connManager, current conn num=%d", conn.GetServer().GetServerType(), conn.GetConnID(), cm.Len())
}
